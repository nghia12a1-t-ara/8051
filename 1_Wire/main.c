#include "regx52.h"

#define Skip_ROM				0xCC
#define Convert_T				0x44
#define Read_ScratchPad	0xBE

sbit PIN = P1^0;

unsigned char code Code7Seg[] = {0xC0, 0xF9, 0xA4, 0xB0, 0x99, 0x92, 0x82, 0xF8, 0x80, 0x90};

void T0_delay_ms(unsigned int t)
{
	unsigned int load_value;
	load_value = 65536 - t;
	TH0 = load_value >> 8;
	TL0 = load_value & 0x00FF;
	
	TR0 = 1;
	while(!TF0);
	TR0 = 0;
	TF0 = 0;
}

unsigned char DS18B20_Reset()
{ 
	unsigned char result;
	PIN = 0; 
	T0_delay_ms(480);
	PIN = 1;
	T0_delay_ms(70);
	
	result = PIN;
	T0_delay_ms(410);
	return result;			//tra ve 1 thi loi, 0 thi OK
}	

void DS18B20_Write1()
{
	PIN = 0;
	T0_delay_ms(6);
	PIN = 1;
	T0_delay_ms(64);
}

void DS18B20_Write0()
{
	PIN = 0;
	T0_delay_ms(60);
	PIN = 1;
	T0_delay_ms(10);
}

void DS18B20_Write_Bit(unsigned char b)
{
	if(b==1)
		DS18B20_Write1();
	else
		DS18B20_Write0();
}

unsigned char DS18B20_Read_Bit()
{
	unsigned char result;
	PIN = 0;
	T0_delay_ms(6);
	
	PIN = 1;
	T0_delay_ms(9);
	
	result = PIN;
	T0_delay_ms(55);
	
	return result;
}

void DS18B20_Write_Byte(unsigned char b)
{
	unsigned char i = 8;
	while(i--)
	{
		DS18B20_Write_Bit(b & 0x01);
		b >>= 1;
	}
}

unsigned char DS18B20_Read_Byte()
{
	unsigned char i = 8, result = 0;
	while(i--)
	{
		result >>= 1;
		result |= (DS18B20_Read_Bit() << 7);		//vi no doc LSB truoc 
	}
	return result;
}

void Delay_ms(unsigned int t)
{
 	unsigned int x,y;
	for(x=0;x<t;x++)
	{
	 	for(y=0;y<=123;y++);
	}
}

void main()
{	
	unsigned int temp;
	unsigned char int_part, decimal_part;
	unsigned char chuc, dvi;
	
	TMOD &= 0xF0;
	TMOD |= 0x01;
	while(1)
	{
		while(DS18B20_Reset());
		DS18B20_Write_Byte(Skip_ROM);
		DS18B20_Write_Byte(Convert_T);
		
		Delay_ms(750);
		while(DS18B20_Reset());
		DS18B20_Write_Byte(Skip_ROM);
		DS18B20_Write_Byte(Read_ScratchPad);
		
		temp = DS18B20_Read_Byte();
		temp = (DS18B20_Read_Byte() << 8) | temp;
		
		int_part = temp >> 4;
		chuc = int_part / 10;
		dvi = int_part % 10;
		
		decimal_part = temp & 0x000F;
		decimal_part = decimal_part*10/16; 	//lay chu so dau cua phan thap phan
		
		P0 = Code7Seg[chuc];
		P2 = Code7Seg[dvi];
		P3 = Code7Seg[decimal_part];	
	}
}
